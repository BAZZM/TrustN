...
| Accept/decline connection requests (inner + secondary introductions) | Done | Intermediary-first secondary intros; inbox vs sent (`scope`); finalized intros are **inner** with provenance; graph **acquired inner** styling via `peer_introduced` |
| Connections trust graph: discovery intro UX | Done | `ConnectionsGraph`: outer-ring tap + **+** chip opens card; **Request intro** / **Request discussion** (placeholder); dashed wireframe when a pending intro already exists for viewer + focused intermediary + target |
...
- `GET /api/connections` – list with strength, `circle_type`, and `peer_introduced` (introduction-acquired inner)  
- `POST /api/connection-requests` – create inner or secondary introduction request  
- `GET /api/connection-requests` – inbox/sent/all (`scope`); `POST /api/connection-requests/:id/respond` – accept/decline and dual-stage approve actions  
...
- **users** – `theme_id`, `large_text`, `high_contrast`, `locale`, `notifications_connection_requests`, `introductions_completed_count`, optional `instance_id`  
- **connections** – `strength` (integer), `circle_type` (inner/secondary), optional `introduced_via_request_id` (introduction-acquired inner only)  
- **access_requests** – `circle_type`, dual timestamps for secondary intros, accept/decline flow  
